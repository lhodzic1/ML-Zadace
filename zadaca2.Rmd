...
model_tuning <- svm(formula = RainTomorrow ~ ., data = podaci_train, kernel = "polynomial", cost=10, gamma=1)
predictions <- predict(model_tuning, newdata = subset(podaci_test, select = -
c(RainTomorrow)))
cm <- confusionMatrix(data = predictions, reference = podaci_test$RainTomorrow)
cat("Accuracy:", cm$overall[1])
...
#prediction <- predict(model_tuning, newdata = subset(weather_data_test, select = -
#c(RainTomorrow)))
...
tuning <- tune.svm(formula = WeatherStability ~ .,
 x = subset(podaci, select = -c(WeatherStability)),
 y = podaci$WeatherStability,
 cost = c(1, 5, 10, 25, 50), gamma = c(0.001, 0.01,
0.1, 1))
...
```{r}
best <- svm(formula = WeatherStability ~ ., data = podaci_train, cost = 25, gamma = 0.1)
predictions_best <- predict(best, newdata = subset(podaci_test, select =-c(WeatherStability)))

plot(podaci_test$WeatherStability, podaci_test$MinTemp, pch = 16)
points(predictions, podaci_test$MinTemp, col = "red", pch = 3)
points(predictions_best, podaci_test$MinTemp, col = "blue", pch = 4)

RMSE <- rmse(podaci_test$WeatherStability, predictions_best)
cat("RMSE:", RMSE, "\n")

r2 = R2(podaci_test$WeatherStability, predictions_best, form = "traditional")
cat("R2:", r2, "\n")

mae_metrika <- mae(podaci_test$WeatherStability, predictions_best)
cat("MAE: ", mae_metrika, "\n")
```
...
begging_svm <- function(podaci, podaci_test, k)
{
 ...
 model <- svm(formula = WeatherStability ~ ., data = train_data, cost = 25, gamma = 0.1)
 
  predictions_best <- predict(model, newdata = subset(podaci_test, select =-c(WeatherStability)))
   
   lista <- append(lista, list(predictions_best))
...
```{r}
lista_svm <- begging_svm(podaci, podaci_test, 30)
pred <- predict_bagging(podaci, podaci_test, lista_svm, 30)

RMSE <- rmse(podaci_test$WeatherStability, pred)
cat("RMSE:", RMSE, "\n")

r2 = R2(podaci_test$WeatherStability, pred, form = "traditional")
cat("R2:", r2, "\n")
